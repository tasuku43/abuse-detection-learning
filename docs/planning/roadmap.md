...
## Phase 10: Scoring API and Local Append-only Log Simulation

目的: Scoring API を純粋なスコアリング境界として立て、S3 append-only log の考え方をローカルファイルで小さく再現する。

本番設計では、scheduled job が Scoring API を呼び、Scoring API は `ScoreResult` だけを返します。candidate 化は Scoring API の責務ではなく、後続の candidate producer の責務です。このフェーズでは、HTTP Scoring API を Docker で起動できる形にしつつ、実 S3 ではなく `data_lake/` 配下の JSONL で履歴ログの考え方を学びます。

### 作るもの

* `src/abuse_detection/local_log_store.py`
* `src/abuse_detection/scoring_api.py`
* `Dockerfile`
* `scripts/build_score_results.py`
...
* Scoring API は score だけを返し、candidate や action を作らないこと
...
* Scoring API が `ScoreResult` を返し、`ActionCandidate` を返さない
* feature rows から Scoring API 経由で score_results JSONL を出力できる
