...
4. simple ML model との比較
5. score calibration

### 5.1 scoring_fn の versioning
...
### 5.4 simple ML model との比較

目的: 手書き scoring_fn と単純なMLモデルを比較する。

候補:

* logistic regression
* decision tree
* random forest などの軽いモデル

完了条件:

* 手書き scoring_fn と ML model の違いを説明できる
* 同じ feature row を使って比較できる
* ML model を使う場合も evaluation harness は再利用できることを確認できる
