...
flowchart LR
  ...
  FB --> FR[(in-memory feature rows)]:::table
...
## 2. 全体像: Evaluation Harness の5レイヤー

評価基盤は、次の5つのレイヤーに分けて考える。

1. Evaluation Dataset Layer（評価データセット作成レイヤー）
2. Point-in-Time Feature Layer（時点特徴量生成レイヤー）
3. Scoring Experiment Layer（スコアリング実験レイヤー）
4. Evaluation Metrics Layer（評価指標レイヤー）
5. Error Analysis / Improvement Layer（誤差分析・改善レイヤー）

```mermaid
flowchart LR
  classDef dataset fill:#E8F3FF,stroke:#2F80ED,color:#1F2937
  classDef feature fill:#E8FAF0,stroke:#219653,color:#1F2937
  classDef scoring fill:#FFF4E5,stroke:#F2994A,color:#1F2937
  classDef metrics fill:#FFF7CC,stroke:#B7791F,color:#1F2937
  classDef improve fill:#FFE8EC,stroke:#D64562,color:#1F2937
  classDef artifact fill:#F3E8FF,stroke:#7C3AED,color:#1F2937

  subgraph EH[Evaluation Harness]
    subgraph L1[Evaluation Dataset Layer]
      LH[(label_events_manual)]:::artifact
      NS([Negative Sampling]):::dataset
      ET[(evaluation_targets)]:::artifact
      LH --> ET
      NS --> ET
    end

    subgraph L2[Point-in-Time Feature Layer]
      FB([Feature Builder]):::feature
      FS[(Feature source events and attributes)]:::artifact
      FR([in-memory feature rows]):::feature
      ET --> FB
      FS --> FB
      FB --> FR
    end

    subgraph L3[Scoring Experiment Layer]
      SC([Scorer / scoring_fn / model]):::scoring
      SR[(scored_rows)]:::artifact
      FR --> SC
      SC --> SR
    end

    subgraph L4[Evaluation Metrics Layer]
      TS([Threshold sweep]):::metrics
      MT[/Precision / Recall / TP / FP / FN/]:::metrics
      SR --> TS
      TS --> MT
    end

    subgraph L5[Error Analysis / Improvement Layer]
      EA[/Error analysis / score buckets/]:::improve
      IMP([Improve scorer / features / labels]):::improve
      SR --> EA
      MT --> IMP
      EA --> IMP
    end
...
Evaluation Dataset Layer（評価データセット作成レイヤー）は、manual label、reversal handling、negative sampling から、評価対象である `entity_id + as_of_time + label_value` を作る。ここでは feature generation も scoring も行わない。

Point-in-Time Feature Layer（時点特徴量生成レイヤー）は、`evaluation_targets` と feature source を受け取り、`as_of_time` 時点で見えていた情報だけから feature row を作る。feature row は scorer との契約であり、必ずしも永続テーブルである必要はない。評価実行中に Feature Builder が生成し、オンメモリで scorer に渡す中間表現として扱える。

Scoring Experiment Layer（スコアリング実験レイヤー）は、feature row を scorer に渡して `risk_score` を出す。ここでは label を見ない。Rule-based scorer、ML model、remote scoring client は、評価基盤から見ると同じ `feature row -> risk_score` のインターフェースで扱う。

Evaluation Metrics Layer（評価指標レイヤー）は、score と label を突き合わせ、threshold sweep、precision / recall、TP / FP / FN、rolling window evaluation、calibration などを計算する。ここで初めて label を使う。

Error Analysis / Improvement Layer（誤差分析・改善レイヤー）は、false positive、false negative、score bucket、drift を観察し、feature、scorer、label、negative sampling の改善仮説へつなげる。

この5レイヤーに分けると、評価基盤と運用スコアリングの関係も整理しやすくなる。両者に共通する核は `entity_id + as_of_time -> Feature Builder -> feature row -> Scorer -> score` である。評価基盤はその後に `score + label -> metrics / improvement` へ進み、運用スコアリングは `ScoreResult -> Decision Policy -> ActionCandidate` へ進む。
...
### 5.4 Feature は都度 recompute できるようにする
...
flowchart TD
  ...
  REV --> LH
  FSE --> FR([Feature Builder input]):::role
  FSA --> FR
...
## 7. Evaluation Dataset Layer: Label Events
...
## 8. Evaluation Dataset Layer: Negative Sampling
...
## 9. Point-in-Time Feature Layer: Feature Rows

feature row は、ある対象のある時点における特徴量を1行にまとめたものである。評価基盤では、positive label と negative sample を合わせて evaluation target を作り、それぞれに対して Feature Builder が feature row を都度生成する。

この feature row は、必ずしも事前にバッチ生成されてDBに保存されたテーブルではない。中心となる考え方は、`evaluation target -> Feature Builder -> in-memory feature row -> scorer` である。fixture CSV や SQLite で作る `feature rows` は、この中間出力を学習・検証しやすくするために固定した教材として扱う。
...
flowchart LR
  ...
  FG --> FR([in-memory feature rows]):::process
  FR --> SC([Scorer]):::scorer
```
...
## 10. Point-in-Time Feature Layer: dbt Skeleton と SQLite Warehouse Simulation
...
flowchart TD
  ...
  LAB --> TGT
  TGT --> FCT[(fct_abuse_feature_rows<br/>simulation output)]:::table
  STG --> FCT
...
## 11. Scoring Experiment Layer と Evaluation Metrics Layer

Evaluation Harness は、scorer を過去データや fixture に適用し、性能を測る実行基盤である。これはモデルそのものではない。モデルやルールを育てるための実験装置である。

評価ハーネスは、Feature Builder が生成した feature rows、または fixture / SQLite simulation として固定した feature rows を受け取り、feature schema を検証し、scoring function を適用して risk score を付ける。その後、threshold を動かし、precision / recall / TP / FP / FN を計算し、false positive と false negative を確認する。
...
flowchart LR
  ...
  FR([feature rows<br/>builder output or fixture]):::process --> VAL{{Schema validation}}:::process
...
## 12. Error Analysis / Improvement Layer
...
## 13. Notebook Workflow

Notebook は、運用システムではない。評価基盤における notebook は、評価ワークフローを手で回すための作業台である。

Notebook は、評価対象 window を決め、Feature Builder の出力または fixture として固定した feature rows を読み込み、scorer を選び、score を付け、threshold sweep を実行し、metrics を表示し、FP/FN を見て改善候補をメモする場所である。
...
flowchart LR
  ...
  DBT([dbt / SQL feature generation]):::code --> FR([feature rows<br/>simulation output]):::table
...
## 14. Scoring Experiment Layer: Rule-based Scorer と ML Baseline
...
評価基盤が整った後で、logistic regression のような単純な ML baseline を追加する。ML baseline は、同じ Feature Builder 出力、または同じ fixture として固定した feature rows を使って学習し、risk score を返す。
...
flowchart TD
  ...
  FR([feature rows<br/>builder output or fixture]):::table --> RBS([RuleBasedScorer]):::scorer
...
## 15. Evaluation Metrics Layer: Score Source / Versioning と Evaluation Results
...
## 16. Evaluation Metrics Layer: Rolling Window Evaluation と Calibration
...
そのためには、label、feature、score、metrics、error analysis を分離する必要がある。`label_events_manual` は manual review による判断に基づく教師ラベル、`evaluation_targets` は評価対象の `entity_id + as_of_time`、feature row は `as_of_time` 時点で見えていた情報から Feature Builder が作る中間出力、scorer は feature row から risk_score を返す部品、threshold sweep は score を decision に変換する前の性能確認、error analysis は FP/FN を見て改善仮説を立てるための作業である。
